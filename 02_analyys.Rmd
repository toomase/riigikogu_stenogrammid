...
```{r emotsioon_sonavottudes, fig.height = 9, fig.width = 8}
...
```{r sonavottude_emotsioon_graafikul, fig.height=5, fig.width=7}
...
Lisan ühe näite positiivse sõnavõtu kohta, kus domineerivad positiivsed sõnad:
...
```{r salvesta_silpide_arv}
# salvesta ja lae silpide arvud
# sonavotud_tidyed_silbid %>%
#     select(kuupaev, nimi, indeks_sonavott, sentence, indeks_sentence, word, silpide_arv) %>%
#     save(file = "output/sonavotud_tidyed_silbid.RData")

load(file = "output/sonavotud_tidyed_silbid.RData")
```
...
```{r sonavotud_smog_graafikul, fig.height=5, fig.width=7}
sonavotud_smog %>% 
...
```{r iseloomulikud_sonad_ylevaade}
glimpse(sonavotud_tidyed)
```
...
Selleks, et tuvastada igale Riigikogu liikmele kõige iseloomulikumad sõnad kasutan mõõdikut *tf-idf* (term frequency–inverse document frequency). Sisuliselt aitab see tuvastada sõnu, mis on kõigi Riigikogulaste sõnavõttudes kokku suhteliselt haruldased, aga konkreetse inimese puhul sagedasemad. Sama meetodit kasutasin ka oma eelmises postituses [http://eilat.ee/2016-11-19-presidentide-koned/](Eesti presidentide kõned).
...
riigikogulaste_iseloomulikud_sonad %>%
    ...
    labs(title = "Riigikogu liikmetele top 5 iseloomulikku sõna",
...
### Kuidas?
Tehniliselt sarnaneb antud postitus paljuski minu eelmise postitusega [http://eilat.ee/2016-11-19-presidentide-koned/](Eesti presidentide kõned). Seetõttu ei hakka ma detaile siin kordama. 
